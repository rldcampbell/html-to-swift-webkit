...
- Map WebKit downloads and common generated `blob:`/`data:` exports to a macOS save panel.
...
The generated app should not expose arbitrary native APIs to JavaScript in V1. It may expose a narrow save-file bridge for generated `a[download]` links using `blob:` or `data:` URLs.
...
- Use WebKit's native download flow when `WKNavigationAction.shouldPerformDownload` is true or a response cannot be displayed.
- Show `NSSavePanel` for native WebKit downloads.
- Intercept normal `a[download]` clicks and common programmatic `anchor.click()` exports for `blob:` and `data:` URLs with a generated `WKUserScript`, then save the generated bytes through `NSSavePanel`.
...
- Native WebKit downloads should prompt with a macOS save panel.
- `a[download]` exports backed by `blob:` or `data:` URLs should prompt with a macOS save panel for normal clicks and common programmatic `anchor.click()` exports.
- The generated bridge should pass only filename, MIME type, and base64 file bytes to Swift.
- If an app uses a highly custom export mechanism, it may still need app-specific handling.

Future improvement:

- Add tests using the sample HTML app.
- Add progress UI for larger native downloads.
- Add explicit JavaScript helper APIs only if real apps need them.
...
- Limit the generated save bridge to `a[download]` links with `blob:` or `data:` URLs.
- Do not enable broad file-system access.
- Block or externally open unexpected HTTP/HTTPS navigation.
- Do not modify the source HTML file.
...
- The generated Swift app maps WebKit downloads and `a[download]` `blob:`/`data:` exports to `NSSavePanel`.
...
- Clicking "Export JPG" opens a macOS save panel.
...
<body>
  <main>
    ...
    <button id="export" type="button">Export JPG</button>
  </main>
  <script>
    ...
    exportButton.addEventListener("click", () => {
      ...
      context.fillText("Sample HTML App", 64, 120);
      context.font = "32px Arial, sans-serif";
      context.fillText(`Clicked ${clicks} ${clicks === 1 ? "time" : "times"}`, 64, 184);
      context.fillText(new Date().toLocaleString(), 64, 246);
      ...
      canvas.toBlob((blob) => {
        ...
        link.href = url;
        link.download = "sample-export.jpg";
        link.click();
...
- Native file open dialogs and richer app-specific save APIs.
